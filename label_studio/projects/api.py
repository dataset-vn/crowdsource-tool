...
class TasksListAPI(generics.ListCreateAPIView,
    ...
    def get_queryset(self):
        ...
        current_user_id = self.request.user.id
        current_user_role = ProjectMember.objects.filter(
            project_id=project.id, user_id=current_user_id)[0].role
        print("CURRENT USER ROLE: ", current_user_role)
        tasks = Task.objects.filter(Q(project=project) & (
            Q(annotator_assigned_id=current_user_id) | Q(reviewer_assigned_id=current_user_id)))
