...
export const MemberListSearchSetting = ({
	onSelect,
	selectedUser,
	defaultSelected,
	projectID,
	...
	const fetchMembers = async function (searchValue) {
		...
		if (searchValue != "") {
			...
			if (result.length == 0)
				setSearchNoti("<b>Không tìm thấy người dùng nào!</b>");
	...
	const selectUser = useCallback(
		(user) => {
			if (selectedUser?.id === user.id || usersList.length === 0) {
				...
				isUserMember(user.id, projectID).then((res) => {
					user.isMember = res;
					onSelect?.(user);
				});
			}
	...
	return (
		<Block name='people-list'>
			<Elem name='search'>
				<input
					type='text'
					style={{ width: "30%", border: "2px solid" }}
					placeholder={t("MemberLSetting.search")}
									...
									<Elem name='field' mix='role'>
										{isCurrentlyActive(user.last_activity) ? (
											<b style={{ color: "#31a24c" }}>
												{t("MemberLSetting.online")}
											</b>
										) : (
											<b style={{ color: "grey" }}>
												{t("MemberLSetting.offline")}
											</b>
										)}
									</Elem>
