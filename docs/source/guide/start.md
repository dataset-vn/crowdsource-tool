...
meta_description: Dataset Documentation for starting Dataset and configuring the environment to use Dataset with your machine learning or data science project. 
---

After you install Dataset, start the server to start using it. 

```bash
label-studio start
```

By default, Dataset starts with a SQLite database to store labeling tasks and annotations. You can specify different source and target storage for labeling tasks and annotations using Dataset UI or the API. See [Database storage](storedata.html) for more.

## Command line arguments for starting Dataset
...
Some available commands for Dataset provide information or start the Dataset server:

| Command |  Description |
| --- | ---- |
| `label-studio` | Start the Dataset server. |
| `label-studio -h` `label-studio --help` | Display available command line arguments. |
| `label-studio init <project_name> <optional_arguments>` | Initialize a specific project in Dataset. |
| `label-studio start <project_name> --init <optional_arguments>` | Start the Dataset server and initiliaze a specific project. |
| `label-studio reset_password` | Reset the password for a specific Dataset username. See [Create user accounts for Dataset](signup.html). |
| `label-studio shell` | Get access to a shell for Dataset to manipulate data directly. See documentation for the Django [shell-plus command](https://django-extensions.readthedocs.io/en/latest/shell_plus.html). |
| `label-studio version` | Show the version of Dataset and then terminates.

The following command line arguments are optional and must be specified with `label-studio start <argument> <value>` or as an environment variable when you set up the environment to host Dataset:

| Command line argument | Environment variable | Description |
| --- | ---- | ---- |
| `-b`, `--no-browser` | N/A | Do not automatically open a web browser when starting Dataset. |
...
| `-d` `--debug` | N/A | Enable debug mode for troubleshooting Dataset. |
| `-c` `--config` | `CONFIG_PATH` | Deprecated, do not use. Specify the path to the server configuration for Dataset. |
| `-l` `--label-config` | `LABEL_STUDIO_LABEL_CONFIG` | Path to the label configuration file for a specific Dataset project. See [Set up your labeling project](setup.html). |
...
| `--log-level` | N/A | One of DEBUG, INFO, WARNING, or ERROR. Use to specify the logging level for the Dataset server. |
| `-p` `--port` | `LABEL_STUDIO_PORT` | Specify the web server port for Dataset. Defaults to 8080. See [Run Dataset on localhost with a different port](start.html#Run-Label-Studio-on-localhost-with-a-different-port) on this page. |
| `--host` | `LABEL_STUDIO_HOST` | Specify the hostname to use to generate links for imported labeling tasks or static loading requirements. Leave empty to make all paths relative to the root domain. For example, specify `"https://77.42.77.42:1234"` or `"http://ls.example.com/subdomain/"`. See [Run Dataset with an external domain name](start.html#Run-Label-Studio-with-an-external-domain-name) on this page. |
| `--cert` | `LABEL_STUDIO_CERT_FILE` | Certificate file to use to access Dataset over HTTPS. Must be in PEM format. See [Run Dataset with HTTPS](start.html#Run-Label-Studio-with-HTTPS) on this page. | 
...
## Run Dataset on Docker with a host and subdomain
...
<!--
## Run Dataset on the cloud using a different cloud provider
To run Dataset on the cloud using a cloud provider such as Google Cloud Services (GCS), Amazon Web Services (AWS), or Microsoft Azure, 
-->
