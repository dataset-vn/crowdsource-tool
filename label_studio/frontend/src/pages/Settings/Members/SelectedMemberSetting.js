		...
		<Block name='user-info'>
			...
			<Elem tag='p' name='status'>
				{isCurrentlyActive(user.last_activity) ? (
					<b style={{ color: "#31a24c" }}>{t("SelectedMember.online")}</b>
				) : (
					<b style={{ color: "grey" }}>{t("SelectedMember.offline")}</b>
				)}
			</Elem>

			<Elem tag='p' name='last-active'>
				{t("SelectedMember.time") /*Hoạt động lúc :*/}{" "}
							...
							: (
								<Space>
									<select
										...
										name='role_member'>
										{Object.keys(ROLE_MEMBER).map((i) => (
											<option value={i} style={{ textAlign: "center" }}>
												{i}
											</option>
										))}
									</select>
